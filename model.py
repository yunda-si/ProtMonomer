# -*- coding: utf-8 -*-
"""
Created on Wed May 20 16:28:07 2024
@author: yunda_si
"""
    ...
    def forward(self, protein):
        ...
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16, enabled=True):
            ...
            for idx_layer, layer in enumerate(self.msa_encoder):
                for idx, sub_layer in enumerate(layer.msa_encoder):
                    msa, pair = sub_layer(msa, pair)
        ...
        for i_struc_cycle in range(protein["num_structure_recycle"]):
            if i_struc_cycle > 0:
                struc_emb = self.trans_seq(struc_emb)
            coords, struc_emb, quat = self.structure_block(struc_emb, msa, pair, coords, quat)

            if i_struc_cycle in protein['plddt_idx']:
                with torch.autocast(device_type='cuda', dtype=torch.bfloat16, enabled=True):
        ...
        pred_coords = torch.permute(torch.stack(pred_coords), [0, 1, 3, 2, 4]) * 10
        pred_plddts = torch.stack(pred_plddts)
        # pred_pae = torch.stack(pred_pae)

        log_memory(pred_plddts.device)
