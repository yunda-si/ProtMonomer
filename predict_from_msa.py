...
def predict(cfg, dataloader):
    ...
    model = ProtMonomer(blocks_msa_encoder=cfg.model.blocks_msa_encoder,
                        ...
                        split_res=cfg.inference.split_res,
    ...
    for d, monomer_dict in enumerate(dataloader):
        ...
        f = cpu_pool.submit(cpu_store,
                            ...
                            pred_lddts[idx_maxplddt,0].squeeze(),
...
if __name__ == '__main__':
    ...
    parser.add_argument('-split_res', '--split_res',
                        default=0,
                        type=int,
                        help='split_res',
                        required=False)
    ...
    parser.add_argument('-last', '--last',
                        action='store_true',
                        help='save structure idx',
                        required=False)

    parser.add_argument('-maxhamming_bin', '--maxhamming_bin',
                        default='/protmonomer/bin/maxhamming',
                        type=str,
                        help='path to maxhamming',
                        required=False)
    ...
    save_last = args.last
    maxhamming_bin = args.maxhamming_bin
    ...
    if split_res>0:
        cfg.inference.split_res = split_res
    if split_atom>0:
    ...
    if msa_file:
        ...
        targets_list=[os.path.join(msa_path,i) for i in os.listdir(msa_path) if i.endswith('.a3m')]
    else:
