...
class IPA(nn.Module):
    ...
    def forward(self, seq, pair, coords, basis, atom_idx=None):
        ...
        if atom_idx == None:
            attn = torch.einsum('bijc, ach -> bhaij', self.norm_pair(pair), self.trans_pair)
        else:
            attn = torch.einsum('bijc, ach -> bhaij', self.norm_pair(pair), self.trans_pair[atom_idx])

        batch, num_atom, num_residue, _ = seq.shape

        q = self.q_seq(seq).view(batch, num_atom, num_residue, self.nhead, self.head_dim) * self.scale_q
        k = self.k_seq(seq).view(batch, num_atom, num_residue, self.nhead, self.head_dim)

        attn += torch.einsum('baihc,bajhc -> bhaij', q, k)
        ...
        attn += qp_attn_weights * head_weights
        attn *= 3 ** -0.5
        attn = attn.softmax(-1)
        ...
        return self.dropout_module(self.linear_out(torch.flatten(o3, -3, -1)))
...
class IPAATOM(nn.Module):
    ...
    def forward(self, seq, coords, basis):
        ...
        attn = torch.einsum('bilhc,bjlhc -> bhlij', q, k)
...
class StructureBlock(nn.Module):
    def __init__(self,
        ...
        self.transition = StructureModuleTransition(
                                                    in_channels=atom_channel,
                                                    dropout_p=dropout_p
                                                    )

    def forward(self, struc_emb, msa, pair, coords, basis):
        ...
        if self.split_atom != None:
            for atom_idx in torch.split(torch.arange(self.num_atom), self.split_atom):
                struc_emb[:, atom_idx] += self.ipa(struc_emb[:, atom_idx], pair, coords[:, atom_idx], basis, atom_idx=atom_idx)
        else:
...
class StructureModule(nn.Module):
    ...
    def __init__(self,
        ...
        self.coords_out = nn.Sequential(
                                        nn.Linear(atom_channel, atom_channel, bias=True),
    ...
    def forward(self, struc_emb, msa, pair, ori_coords, quat):
        ...
        for idx_layer, layer in enumerate(self.structure_block):
            struc_emb = layer(struc_emb, msa, pair, ori_coords, basis)
