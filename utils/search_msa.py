...
def search_msa(seq_file,
    ...
    key = 'bfd'
    msa_file = os.path.join(save_path, f'{seq_id}_{key}.a3m')
    ...
    key = 'uniref90'
    msa_file = os.path.join(save_path, f'{seq_id}_{key}.a3m')
    sto_file = os.path.join(save_path, f'{seq_id}_{key}.sto')
    masksto_file = os.path.join(save_path, f'{seq_id}_{key}.masto')
    a2m_file = os.path.join(save_path, f'{seq_id}_{key}.a2m')
    ...
    msa_file = os.path.join(save_path, f'{seq_id}.a3m')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-bfd', '--bfd_path', type=str, help='BFD/bfd_metaclust_clu_complete_id30_c90_final_seq.sorted_opt',required=True)
    parser.add_argument('-uniref90', '--uniref90_path', type=str, help='database/uniref90.fasta',required=True)
    parser.add_argument('-mgnify', '--mgnify_path', type=str, help='database/mgy_clusters.fa',required=True)

    args = parser.parse_args()
    search_msa(args.seq_file,
               args.save_path,
               args.bfd_path,
               args.uniref90_path,
               args.mgnify_path)